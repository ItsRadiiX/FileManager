...
import nl.bryansuk.foundationapi.events.FileReloadEvent;
...
public class FileHandler<T> extends Handler {
    ...
    public boolean onReload() {
        if (isNewVersionAvailable()){
            ...
            read();
            Bukkit.getPluginManager().callEvent(new FileReloadEvent(getFile().getName()));
            return true;
        }
        return false;
    }
    ...
    private T checkIfObjectExists(boolean fileExists) {
        ...
        if (defaultResource) {
            try {
                T defaultContent = converter.readFromFile(FileManager.getPlugin().getResource(getPath()), typeReference);
